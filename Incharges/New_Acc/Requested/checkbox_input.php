...
<body>
    ...
    if (!function_exists('generateCheckboxInput')) {
        function generateCheckboxInput($prn, $Accountant_status)
        {
            echo
            '
                <form id="myForm_' . $prn . '" class="d-flex flex-row" action="update_status.php" method="post">
                    <input type="checkbox" name="Accountant_status[' . $prn . ']" value="1" ' . ($Accountant_status ? 'checked' : '') . ' ">
    ...
    <script>
        function submitForm(prn) {
            ...
            var checkboxValue = document.getElementsByName("Accountant_status[" + prn + "]")[0].checked ? 1 : 0;
